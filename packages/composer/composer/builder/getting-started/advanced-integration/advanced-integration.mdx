...
We have an entire [article on HMAC](/composer/builder/best-practices/secure-webhook-endpoints-hmac.mdx), but the quick summary is that you and the third party (GitHub or Zendesk) know some secret, and the third-party uses that secret to generate a unique hash of the message they sent.
